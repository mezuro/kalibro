...
public class RepositoryXmlTest extends DtoTestCase<Repository, RepositoryXml> {
	...
	public void defaultConstructorShouldDoNothing() {
		Repository respository = newDtoUsingDefaultConstructor().convert();
		assertNull(respository.getType());
		assertNull(respository.getAddress());
		assertTrue(respository.getUsername().isEmpty());
		assertTrue(respository.getPassword().isEmpty());
	}
